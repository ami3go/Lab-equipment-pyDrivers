...
## Standard Evidence, Structured Test Reports, JSON, JSONL, CSV, Traces, and Diagnostic Bundles
...
This specification defines the mandatory logging, evidence, traceability, and result-export requirements for the LPDS Python Instrument Driver Platform.
...
3. what operation or public API method was executed;
...
LPDS-008 defines the **project-wide evidence schema and preservation rules**. It does not replace an automation-framework adapter's native reporting, LPDS-009 testing requirements, LPDS-019 call and protocol conformance, LPDS-010 review requirements, or LPDS-011 release evidence. It standardizes how those activities record and exchange evidence.

---

## 2. Goals

The LPDS logging and evidence system shall:
...
- provide deterministic correlation from a conformance test — run directly against the driver's plain Python API, or through any automation-framework adapter — to driver operations, protocol traces, device responses, measurements, errors, cleanup, reviews, and releases;
...
LPDS-008 does not define:

- the complete public Python API;
- vendor-specific protocol syntax;
- transport implementation;
- the exception hierarchy and error taxonomy owned by LPDS-007;
...
Where another LPDS specification defines stricter evidence requirements for its scope, the stricter requirements shall apply.
...
|---|---|---|
| `AUTHORITATIVE_RESULT` | Primary result authority | framework-agnostic test-result JSON/JUnit XML, validated result JSON |
...
| `DERIVED_REPORT` | Human-readable or summarized output | adapter-native HTML report, Markdown summary, PDF report |
...
### 8.2 E1 — Conformance Test Evidence
...
- LPDS-019 public API inventory and protocol vectors;
...
- `suite_id` — one conformance test suite;
- `test_id` — one conformance test;
- `method_id` — one public API method invocation captured by an adapter or test harness, when applicable;
...
- `protocol_vector_id` — one LPDS-019 vector;
...
- authoritative test-result artifact references;
...
### 14.1 Mandatory test-result artifacts

Every formal conformance test run shall produce a framework-agnostic authoritative test-result record containing at least:

- test-run identity and timestamps;
- suite/test identifiers and outcomes;
...
Test-result artifacts shall be stored under the run-specific `test_results/` directory or an equivalent path referenced by the evidence manifest.

### 14.3 Naming

Standard filenames should be retained. When multiple test-runner invocations occur in one run, each invocation shall use a unique subdirectory or filename prefix and shall be listed in the manifest.

### 14.4 JUnit-style xUnit output

CI workflows should generate a JUnit-style `results.xml` when supported by the test runner. JUnit-style xUnit output shall not replace the primary authoritative JSON test-result record.
...
The hook implementation shall not expose secrets or duplicate very large payloads already stored as attachments.

```python
# Plain Python: a test harness registers a callback that the driver-agnostic
# evidence subsystem invokes for every method start/end, independent of any
# automation framework.
def on_method_call(event):
    evidence.record_operation(event)
...
| LPDS intent | Python `logging` level |
|---|---|
| Detailed internal diagnostic | `DEBUG` |
| Normal lifecycle and meaningful operation | `INFO` |
| Recoverable or non-blocking concern | `WARN` |
| Test- or operation-affecting failure | `ERROR` or conformance-test failure |
| Trace-level protocol bytes | dedicated trace artifact or `TRACE` where available |

Protocol traces shall not be embedded only in an adapter-native HTML log when machine-readable or raw trace evidence is required.

### 14.7 Embedded attachments

Adapter-native reports (or any derived HTML/PDF presentation) may link to screenshots, plots, CSV files, traces, or other attachments. The underlying files shall remain independently accessible in the result directory.
...
- the merge tool's name, version, and command shall be recorded;
...
```json
{
  ...
  "producer_id": "keysight_n6700:PSU_MAIN",
  "sequence": 124,
...
- public API method or capability request;
- argument validation;
...
- conformance-test suite and run lifecycle;
...
- originating conformance-test suite/test/method, GUI action, Python call, or automation;
- driver and session alias;
- capability ID and public API method;
- canonical method name and alias used;
- start and end timestamps;
...
- run, correlation, operation, session, suite, test, and method IDs;
- LPDS error category and code defined by LPDS-007;
- exception type;
- concise message;
- causal chain;
...
- automation-framework adapter version(s) in use, if any;
- LPDS platform/core version;
...
Configuration fingerprints shall exclude or normalize secret values according to LPDS-014 while remaining useful for comparing effective non-secret configuration.
...
LPDS-019 is authoritative for public API call and protocol conformance. LPDS-008 defines the common evidence envelope and storage rules.

Protocol evidence shall record, as applicable:

- protocol exchange ID;
- operation ID, method, capability, and vector IDs;
- transport type;
...
Teardown failure shall be visible in the authoritative test-result record, run summary, and cleanup summary.
...
PDF reports are derived evidence. Their source JSON, CSV, test-result XML, and attachment references shall be retained.
...
Large payloads shall not be repeated across Python logs, adapter-native logs, JSONL, and traces. Logs should reference the authoritative attachment.
...
- driver ID;
- session alias;
- suite/test/method ID where available;
- source component.
...
5. finalize authoritative test-result outputs (and any adapter-native reports);
...
The shared LPDS platform or `BaseInstrument` should provide common interfaces equivalent to:
...
Each driver package shall contain or reference the approved LPDS evidence schemas required for its outputs.
...
Shared schemas may be supplied by the LPDS core package when package versions and compatibility are declared.
...
- run IDs are unique;
- operation and correlation IDs propagate through test-execution, driver, protocol, error, measurement, and cleanup evidence;
...
### 37.4 Test-result evidence tests

Verify:

- the framework-agnostic authoritative test-result record (JSON and/or JUnit-style XML) is generated;
- any adapter-native report is generated when required and agrees with the authoritative record;
- test result totals agree with `run_summary.json`;
- setup and teardown failures remain visible;
- merged/post-processed outputs reference original outputs;
...
- configured secrets do not appear in logs, JSON, JSONL, CSV, adapter-native reports, traces, attachments, or diagnostic bundles;
...
- propagate run, correlation, operation, session, test-execution, protocol, and sample identifiers;
...
### Step 7 — Generate authoritative test result and derived reports

- finalize the authoritative test-result record (and any adapter-native reports);
...
```json
{
  ...
  "evidence_refs": [
    ...
    "test_results/results.json#method-00341"
  ]
}
```
...
1.0.0,run-20260727T081530.412Z-7f3a21c8,suite-psu-efficiency,test-vin-12v,corr-step-00045,op-calc-000095,sample-000001,2,2026-07-27T08:15:31.620Z,1.208,lpds_calculation,POWER_EFFICIENCY,dut-psu-01,,EFFICIENCY,81.42,%,CALCULATED,VALID,PASS,,,65.0,,%,,measurements/measurements.csv#row-1,CALC-EFFICIENCY-v1,
```
...
```json
{
  "path": "test_results/results.json",
  "role": "AUTHORITATIVE_RESULT",
  "media_type": "application/json",
  "schema": "lpds.test_result",
  "schema_version": "1.0.0",
  "size_bytes": 184220,
...
```json
{
  "schema": "lpds.run_summary",
  "schema_version": "1.0.0",
  "run_id": "run-20260727T081530.412Z-7f3a21c8",
  "activity": "HIL_CONFORMANCE_TEST",
  ...
  "test_results": {
    ...
    "failed": 0,
    "skipped": 0,
    "authoritative_result": "test_results/results.json",
    "junit_xml": "test_results/results.xml",
    "adapter_reports": ["test_results/adapter_report/report.html"]
  },
  "safety": {
    "safe_start": "VERIFIED",
...
|---|---|---|
| Did a conformance test pass? | framework-agnostic authoritative test-result record (JSON/JUnit XML) | adapter-native HTML report, Markdown summary |
...
- the LPDS evidence principles and classification model;
...
- mandatory framework-agnostic test-result evidence, with optional adapter-native report integration;
