...
**Version:** 1.0  
**Document ID:** LPDS-006  
**Status:** Project requirement  
**Applies to:** All Python code in LPDS Python instrument driver projects

---

## 1. Purpose

This specification defines the mandatory coding standard for Python-based instrument drivers developed under LPDS.
...
The objective is to make every LPDS driver consistent, understandable, testable, diagnosable, and safe to extend by both human developers and AI implementation agents.
...
- driver public API methods and automation-framework adapter implementations;
...
LPDS-006 does not define:

- the mandatory public API method set;
...
Those subjects are defined by the applicable LPDS API, transport, package, test, and AI-contract specifications.
...
- **public API** — exported Python classes, functions, methods, properties, exceptions, and constants intended for external use, including any bindings that an automation-framework adapter re-exports;
...
All LPDS Python code shall follow these principles:
...
4. **Separation of concerns** — automation-framework adaptation, domain logic, transport I/O, protocol serialization, and configuration shall not be unnecessarily coupled.
...
The recommended LPDS default is Ruff format with:
...
| Package | `snake_case` | `keysight_34970` |
...
The driver itself is fully constructible, controllable, and testable from a plain Python session with no automation framework installed; the adapter layer is optional, thin, and added only after the driver is verified on its own public API.
...
- documented adapter-exposure declarations, where an automation-framework adapter re-exports the driver's public methods under that framework's own binding mechanism.
...
- deterministic;
- documented;
- structurally stable, so that any automation-framework adapter can translate them into that framework's own compatible types without ambiguity;
...
The driver's own public API should use the most expressive documented Python type for the result:
...
- all automation-framework adapter methods;
...
## 10. Public API Method Implementation

### 10.1 Method exposure

Public API methods shall be intentionally exposed through the driver's declared public-surface mechanism (Section 8.1).
...
- be readable and self-explanatory as Python identifiers;
- remain consistent across LPDS drivers for equivalent operations, per LPDS-002;
...
- safety constraints;
- a short plain-Python usage example when the call is not self-evident.

Method documentation shall remain compatible with automated API-reference generation (for example Sphinx or mkdocs), and shall provide enough information that an automation-framework adapter can document its own binding by reference rather than by rediscovering behaviour.

### 10.4 Failures reaching the caller

Device and validation failures shall reach every caller — a direct Python caller or a caller going through an automation-framework adapter — as clear failures with actionable messages.
...
- produce equivalent behaviour unless explicitly documented otherwise;
...
- automation-framework adapter classes and methods that ship as part of the same reviewed codebase.
...
A package shall use one consistent structured docstring style. Google style is the recommended LPDS default.
...
def set_output_voltage(
    ...
    *,
    ...
    Example:
        >>> driver.set_output_voltage(1, 5.0)
    """
```
...
```python
# TODO(LPDS-123): Remove compatibility branch after vendor SDK 5.x is no longer supported.
```
...
- use current public method names and signatures;
...
This table uses the Python standard-library `logging` level names (including `WARNING`) because it governs calls into that module. It is distinct from the LPDS-008 §28.1 evidence-level vocabulary (`TRACE`/`DEBUG`/`INFO`/`WARN`/`ERROR`/`CRITICAL`) used for structured evidence records; a `logger.warning(...)` call should be mapped to LPDS-008's `WARN` evidence level when the message is also captured as structured evidence.
...
- logging configuration shall not change globally merely because the driver package was imported.
...
LPDS-007 is the sole normative source for the LPDS exception hierarchy, class names, and error codes. Each driver package shall expose the LPDS-007 base exception:
...
String concatenation distributed across public methods is prohibited when it obscures protocol behaviour.
...
python -m ruff check .
python -m mypy <driver_package_name>
python -m pytest
```
...
- public API methods without user-facing documentation;
...
dependencies = [
  # device-specific runtime dependencies only, e.g. "pyvisa>=1.13".
  # this package shall not depend on any test-automation framework.
]
...
This example follows the LPDS-007 category names; LPDS-007 remains the authoritative source for the complete hierarchy.
