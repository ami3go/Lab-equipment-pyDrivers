...
**Version:** 1.0  
**Document ID:** LPDS-007  
**Status:** Project requirement  
**Applies to:** All LPDS Python instrument driver packages, shared driver components, transport adapters, protocol parsers, simulators, and conformance tests

---

## 1. Purpose

This specification defines the mandatory error, exception, diagnostic, retry, recovery, and failure-propagation behavior for LPDS drivers.
...
3. produces a deterministic and actionable failure message, delivered as a raised Python exception;
...
9. is testable through unit, integration, and LPDS conformance tests;
10. is described in the LPDS-017 AI Driver Contract.
...
- Python exception classes exposed by LPDS drivers;
- validation failures;
...
- exception chaining;
- cleanup failures;
- public exception propagation and failure-message rendering;
- structured diagnostic records;
- logging and evidence requirements;
- redaction of sensitive information;
- error recovery and post-failure state;
- error catalogue requirements in LPDS-017;
- protocol-error and recovery verification under LPDS-019.

### 2.2 Out of scope

LPDS-007 does not define:
...
- **public boundary** — any documented public Python API (method, property, or constructor);
...
Every LPDS driver shall expose one documented root exception named:
...
- a human-readable string (returned by `str()`, matching the canonical public failure message format in §11);
...
Canonical LPDS error codes shall use:
...
|---|---|---|---:|
| `LPDS-CFG-001` | `DriverConfigurationError` | Missing required configuration | No |
| `LPDS-CFG-002` | `DriverConfigurationError` | Invalid configuration value or schema | No |
| `LPDS-CFG-003` | `DriverConfigurationError` | Unsupported configuration combination | No |
| `LPDS-ARG-001` | `DriverArgumentTypeError` | Invalid argument type | No |
| `LPDS-ARG-002` | `DriverArgumentValueError` | Invalid argument value | No |
| `LPDS-ARG-003` | `DriverRangeError` | Value outside supported range | No |
| `LPDS-ARG-004` | `DriverUnsupportedValueError` | Unsupported enum, mode, or option | No |
| `LPDS-ARG-005` | `DriverValidationError` | Conflicting or incomplete arguments | No |
| `LPDS-STA-001` | `DriverPreconditionError` | Operation precondition not satisfied | Conditional |
| `LPDS-STA-002` | `DriverStateError` | Operation invalid in current state | Conditional |
| `LPDS-STA-003` | `DriverOperationUncertainError` | Device-changing operation may or may not have executed | No automatic retry |
| `LPDS-CON-001` | `DriverTransportOpenError` | Connection or session open failed | Conditional |
| `LPDS-CON-002` | `DriverConnectionError` | Driver is not connected | Conditional |
| `LPDS-CON-003` | `DriverConnectionError` | Connection lost | Conditional |
| `LPDS-CON-004` | `DriverConnectionError` | Identity or handshake failed during connection | Conditional |
| `LPDS-TRN-001` | `DriverWriteError` | Transport write failed | Conditional |
| `LPDS-TRN-002` | `DriverReadError` | Transport read failed | Conditional |
| `LPDS-TRN-003` | `DriverTransportClosedError` | Transport is closed | Conditional |
| `LPDS-TRN-004` | `DriverTransportError` | Transport framing or low-level I/O failure | Conditional |
| `LPDS-TMO-001` | `DriverTimeoutError` | Operation exceeded its timeout | Conditional |
| `LPDS-PRT-001` | `DriverMalformedResponseError` | Response format is malformed | Conditional |
| `LPDS-PRT-002` | `DriverIncompleteResponseError` | Response or frame is incomplete | Conditional |
| `LPDS-PRT-003` | `DriverUnexpectedResponseError` | Response type or content is unexpected | Conditional |
| `LPDS-PRT-004` | `DriverChecksumError` | Checksum or integrity validation failed | Conditional |
| `LPDS-PRT-005` | `DriverProtocolSyncError` | Protocol stream synchronization was lost | Conditional |
| `LPDS-DEV-001` | `DriverCommandRejectedError` | Device rejected a command | No unless documented |
| `LPDS-DEV-002` | `DriverDeviceError` | Device error queue or status reports failure | No unless documented |
| `LPDS-DEV-003` | `DriverDeviceBusyError` | Device is temporarily busy | Yes when documented |
| `LPDS-DEV-004` | `DriverUnsupportedOperationError` | Device or model does not support operation | No |
| `LPDS-DEV-005` | `DriverIdentityError` | Connected device identity is not accepted | No |
| `LPDS-RES-001` | `DriverResourceNotFoundError` | Required resource was not found | Conditional |
| `LPDS-RES-002` | `DriverResourceBusyError` | Resource is already in use | Yes when release is expected |
| `LPDS-RES-003` | `DriverResourceConflictError` | Resource allocation conflicts with another operation | No until plan changes |
| `LPDS-SAF-001` | `DriverUnsafeOperationError` | Operation is prohibited by a safety rule | No |
| `LPDS-SAF-002` | `DriverInterlockError` | Required safety interlock is not satisfied | No until condition changes |
| `LPDS-SAF-003` | `DriverLimitViolationError` | Requested operation exceeds a configured safety limit | No |
| `LPDS-DEP-001` | `DriverDependencyError` | Required dependency is missing | No |
| `LPDS-DEP-002` | `DriverDependencyError` | Dependency, SDK, firmware, or runtime version is incompatible | No |
| `LPDS-INT-001` | `DriverInternalError` | Internal invariant failed | No |
| `LPDS-INT-002` | `DriverInternalError` | Unexpected unclassified implementation failure | No |
...
| `method` | Public API method name when known |
...
The canonical public Python exception message format shall be:
...
```text
[LPDS-TMO-001] Get DC Voltage failed: no complete SCPI response was received within 5.000 s. Transport=VISA; Resource=TCPIP0::192.168.0.55::INSTR; Attempt=2/2. Retryable=yes. Recovery=verify the connection and repeat the query.
```

### 11.1 Message rules

1. The error code shall be first.
2. The operation or method shall be named.
...
## 12. Exception Propagation and Fatal-Failure Behavior

### 12.1 Method failure

A public driver method shall fail by raising a `DriverError` or a subclass. Calling code — a plain Python caller, a pytest test, or a framework adapter — shall receive the canonical public failure message via the raised exception.

A method shall not both return a success value and report a driver failure.

### 12.2 Assertion failures versus driver errors

Driver methods that explicitly implement assertions, such as a `voltage_within_limits` check, may raise a dedicated assertion-style exception distinct from `DriverError` (or, when the driver is wrapped by a test-automation adapter, allow that adapter to raise its own framework's native assertion failure) when:

- communication succeeded;
- data was valid;
- the measured value did not meet the assertion.

Communication, parsing, safety, configuration, or state failures shall remain driver exceptions raised as `DriverError` subclasses and shall not be mislabeled as ordinary assertion failures.

### 12.3 Unrecoverable failures

A driver shall signal an unrecoverable condition — for example by raising an exception classified with `recovery_class = NOT_RECOVERABLE` or `EMERGENCY_SHUTDOWN_REQUIRED` (see §16.2) — only when continuing to use the driver instance is unsafe or structurally impossible, such as:

- required emergency shutdown failed;
- the underlying transport or shared driver process is corrupted;
...
When a method fails, it shall not return a partial result unless the API explicitly defines a typed result that contains both partial data and a failure status. Even then, transport and safety failures shall normally raise exceptions.
...
```text
[LPDS-ARG-003] Set Current failed: requested current 12.0 A exceeds the configured maximum of 10.0 A. Channel=1. Retryable=no. Recovery=reduce the requested current or revise the approved safety limit.
```
...
Each exception path shall define the driver state after failure using the canonical session-state enum owned by LPDS-003 §13.1 (`DISCONNECTED`, `CONNECTING`, `CONNECTED`, `CONFIGURED`, `BUSY`, `WAITING`, `RECOVERING`, `ERROR`, `CLOSING`). The driver's LPDS-017 contract shall reference this same enum rather than defining a competing one.

A state-changing exception that leaves the device in a condition requiring reconciliation before further use shall report `ERROR` or `RECOVERING` as applicable, with the recovery classification (Section 16.2) supplying the finer-grained detail LPDS-003's enum does not itself capture.
...
When a failure is documented as recoverable, tests shall perform the documented recovery and then execute a known-good method call.
...
Canonical DriverProtocolError
        ↓
Public failure message (raised exception) + diagnostic record
```
...
A close or disconnect method shall be idempotent unless the device protocol makes this impossible.
...
cause:
  type: "TimeoutError"
  vendor_code: null
correlation_id: "run-or-invocation-correlation-id"
evidence:
...
7. Correlation identifiers shall allow association with the calling test (when run under a test-automation adapter), method invocation, protocol vector, and transport trace.
...
| Level | Use |
|---|---|
| `TRACE` | Raw protocol bytes and other trace-level detail (see LPDS-008 §28.1) |
...
LPDS-008 §28.1 is the sole normative source for the platform's logging-level set and their evidence-level meaning; this table uses LPDS-008's level names (`WARN`, not `WARNING`) for exception- and error-related logging specifically.
...
- exception type;
- driver name and version;
- calling context (automation-framework suite, test, and invocation identifiers supplied by an adapter, or an equivalent caller-supplied identifier) when available;
- operation;
...
### 20.5 Relationship to LPDS-008

The §19 diagnostic record is the canonical representation of one raised exception. When persisted as run evidence, it shall be carried into the LPDS-008 §17 error-evidence record without renaming fields that already have an LPDS-007 name (`code`, `exception_type`, `message`, `retryable`, `recovery_class`/`recovery_action`, `correlation_id`); LPDS-008 adds the surrounding run/session/sequence identifiers and storage rules. LPDS-008 is the sole normative source for how error evidence is stored, correlated, and redacted at the run level.
...
A diagnostic record should identify which fields were redacted without revealing their values. This inline text marker is for human-readable log output; the structured JSON redaction marker used in machine-readable evidence (`{"value": "<REDACTED>", "redacted": true, "reason": "..."}`) is governed by LPDS-008 §29.3.
...
## 23. LPDS-017 Integration

The LPDS-017 AI Driver Contract shall include an error catalogue consistent with this specification.

For each capability or method, LPDS-017 shall identify:
...
An AI agent shall be able to determine from LPDS-017 whether it may retry, reconnect, read back state, ask for operator action, or abort the test.
...
- correct public exception propagation and failure-message rendering;
- traceable protocol and diagnostic evidence.

A protocol failure shall not pass merely because the method raised an exception. The exception class, code, retryability, post-failure state, and recovery behavior shall match the declared contract.
...
For each public method, tests shall cover applicable failures:
...
Automation-framework adapter tests shall verify:

- the adapter surfaces the driver's declared error code and exception type unchanged;
- the failure text delivered to the framework is actionable;
- the result is not misreported as PASS;
- retry and recovery behavior declared by the driver are preserved through the adapter;
...
class DriverError(Exception):
    ...
    def __str__(self) -> str:
        operation = self.method or self.operation or "Driver operation"
...
errors:
  - code: "LPDS-DEV-001"
    ...
    tests:
      - "ERR-DEV-001"
      - "LPDS019-VECTOR-SET-VOLTAGE-DEVICE-ERROR"
    introduced_in: "26.01"
    deprecated: false
```
...
Whenever a public method, transport operation, device error, timeout policy, retry policy, recovery action, or safety rule changes, the same driver revision shall update:

- exception classes;
- error codes and catalogue;
- LPDS-017 capability errors;
- LPDS-019 protocol error vectors;
