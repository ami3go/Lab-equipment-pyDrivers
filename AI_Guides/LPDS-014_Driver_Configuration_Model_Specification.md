...
**Applies to:** All LPDS Python instrument driver packages, shared configuration components, generic operator applications, AI-assisted test generation, and release validation

---

## 1. Purpose

This specification defines the common configuration model for LPDS Python instrument drivers.
...
9. expose configuration operations consistently through the driver's public Python API, to Python applications, generic GUIs, framework adapters, and AI agents;
...
LPDS-014 covers host-side driver configuration, including:
...
- standard public Python configuration methods;
- integration with LPDS capability, AI-contract, GUI, test, and release requirements.
...
- LPDS-018 bench topology, DUT wiring, relay routing, or shared-resource assignments;
...
A setting that changes physical-device non-volatile memory remains a device-facing operation. It shall be declared in LPDS-017, safety-classified, and tested through the applicable LPDS-019 protocol vectors. LPDS-014 only governs the host-side representation and authorization of that operation.
...
- **configuration document** — a complete LPDS-014 JSON object representing one driver profile;
- **schema** — the JSON Schema that defines valid configuration structure, types, constraints, defaults, and LPDS annotations;
...
- **runtime override** — a non-persistent value applied to the current driver instance or named session;
...
Every LPDS driver shall support JSON configuration import and export without optional dependencies.

YAML, TOML, environment-variable files, adapter-specific variable files, and GUI forms may be supported as authoring or integration conveniences, but they shall be converted into the LPDS-014 data model before validation or application.
...
Configuration operations shall return structured, plain Python/JSON-serializable dictionaries and lists. Success shall not be represented only by log text.
...
LPDS implementations shall classify configuration into the following domains.

| Domain | Owner | LPDS-014 treatment |
|---|---|---|
...
| Bench wiring and resource assignments | LPDS-018 bench contract | Referenced, not duplicated as driver truth |
...
Every LPDS-014-conformant driver shall provide, per the LPDS-005 `src/`-layout:
...
1. `src/<driver_name>/resources/configuration/` contains package data used at runtime.
...
9. No user-generated or machine-local profile shall be included in the release archive.
10. The installed driver shall not modify packaged files.

### 6.1 Relationship to LPDS-005

LPDS-005 §6 lists `config/default.json`, `config/example.json`, `config/schema.json`, and `config/schema.lock` as the canonical package artifacts, aligned with LPDS-014. LPDS-014 establishes JSON as the canonical configuration interchange and persistence format; LPDS-005 is authoritative for exactly where these files live in the package tree.
...
A bench/HIL resource template governed by LPDS-018 remains YAML by design; it is not an LPDS-014 configuration document and is unaffected by this section.
...
{
  "lpds014_version": "1.0",
  "schema_id": "<driver_name>.configuration",
...
Each driver configuration schema shall use JSON Schema Draft 2020-12 unless the LPDS project approves a later common dialect.
...
- semantic version information through `x-lpds-schema-version`.

### 8.3 Strictness

The schema shall use `additionalProperties: false` for core LPDS objects unless an approved extension point is explicitly defined.
...
| `x-lpds-apply-mode` | `HOT`, `NEXT_CONNECTION`, `DRIVER_RESTART`, `DEVICE_PERSISTENT` | When a validated change takes effect |
| `x-lpds-risk-level` | `NONE`, `LOW`, `MEDIUM`, `HIGH`, `CRITICAL` | Safety classification |
| `x-lpds-sensitive` | Boolean | Redaction and export policy |
| `x-lpds-secret-reference` | Boolean | Value is a reference, not the secret itself |
| `x-lpds-merge` | `DEEP_MERGE`, `REPLACE`, `APPEND_UNIQUE` | Merge behaviour |
| `x-lpds-requires-disconnected` | Boolean | Active connection must be closed before apply |
| `x-lpds-requires-confirmation` | Boolean | Explicit authorization required |
| `x-lpds-device-write` | Boolean | Applying the value causes a protocol operation |
| `x-lpds-deprecated-since` | Version string | Deprecation start |
| `x-lpds-replacement` | JSON Pointer or setting name | Replacement field |
| `x-lpds-capability` | Capability identifier | LPDS-013/LPDS-017 relationship |
| `x-lpds-environment-variable` | String | Approved environment override |
| `x-lpds-portable` | Boolean | Whether the field is expected to transfer between hosts |

Custom annotations shall be treated as metadata by generic JSON Schema validators and as normative input by LPDS configuration tooling.

`x-lpds-risk-level` uses the same five-tier risk scale defined canonically by LPDS-002 §16.1 (`none`/`low`/`medium`/`high`/`critical`), serialized in upper case to match this schema's other annotation enums; tooling that compares risk levels across LPDS-002, LPDS-013, and LPDS-014 shall compare case-insensitively rather than treating the casing as a semantic difference.
...
An adapter's own input conventions may accept documented equivalent inputs but shall normalize them before validation and export.
...
An adapter's own time-string conventions may be accepted at that adapter's boundary, but canonical configuration shall contain normalized seconds.
...
| `SESSION_OVERRIDE` | Current named driver session only | Test suite or application |
| `INSTANCE_OVERRIDE` | Current driver instance | Constructor or embedding application |
...
7. driver constructor arguments
8. INSTANCE_OVERRIDE
9. SESSION_OVERRIDE
10. explicit method arguments for the current operation
```

An operation-specific method argument shall not be persisted merely because it has the highest runtime precedence.
...
Objects shall deep-merge by key unless the schema annotation `x-lpds-merge` specifies `REPLACE`.
...
LPDS-014 does not use `null` as a generic deletion marker.
...
- a JSON file path;
- a JSON string;
- an already parsed Python dictionary;
...
```json
{"secret_ref": "ENV:LPDS_DEVICE_TOKEN"}
```

```json
{"secret_ref": "OS_VAULT:lpds/device/token"}
```
...
Export may return the configuration as a Python dictionary and may optionally write a JSON file.
...
```text
Windows:
%APPDATA%\LPDS\<driver_name>\

Linux:
${XDG_CONFIG_HOME:-~/.config}/lpds/<driver_name>/
```
...
- the release archive extraction root unless explicitly selected as a project profile directory;
- the current working directory by default;
- a test-output directory unless explicitly requested;
...
Drivers shall not persist every setter method call automatically.
...
The driver shall not change transport settings while another method call is using the same transport.
...
LPDS configuration uses:

- `lpds014_version` — version of this platform model;
...
Plaintext credentials, tokens, passwords, private keys, and certificates shall not be stored in normal LPDS driver profiles.
...
```json
{
  "secret_ref": "ENV:LPDS_DEVICE_PASSWORD"
}
```
...
- not be returned by configuration methods;
- not be included in diagnostics, traces, logs, exceptions, fingerprints, or exports;
...
Every setting that can affect energy, motion, temperature, pressure, relay routing, calibration, protection, or device persistence shall carry appropriate LPDS schema annotations.
...
LPDS-018 remains authoritative for deployed bench limits and interlocks.
...
The public capability shall still enforce LPDS-017 risk, state, and confirmation requirements when called.

### 21.6 Safe startup

Loading a profile at driver construction shall not connect to hardware or energize outputs unless the user explicitly invokes a connection or operation method.

---

## 22. Standard Public Python Configuration API
...
- return the active JSON Schema as a plain Python dictionary.
...
This method shall not access hardware.

### 22.2 `get_driver_default_configuration(redact_sensitive=True)`

Purpose:

- return the validated package default configuration.

This method shall not access hardware or writable user storage.

### 22.3 `get_driver_configuration(scope="EFFECTIVE", alias=None, redact_sensitive=True, include_sources=False)`
...
### 22.4 `validate_driver_configuration(configuration, mode="REPLACE", strict=True, alias=None)`
...
### 22.5 `import_driver_configuration(source, mode="REPLACE", apply=False, persist=False, profile_name=None, strict=True, reconnect=False, allow_device_persistent_changes=False, confirm_high_risk=False, alias=None)`
...
### 22.6 `export_driver_configuration(destination=None, scope="EFFECTIVE", profile_name=None, overwrite=False, alias=None)`
...
### 22.7 `save_driver_configuration(profile_name, scope="EFFECTIVE", overwrite=False, set_active=False, alias=None)`
...
### 22.8 `load_driver_configuration(profile_name, apply=False, reconnect=False, alias=None)`
...
### 22.9 `list_driver_configuration_profiles()`
...
### 22.10 `delete_driver_configuration_profile(profile_name, confirm=False)`

Purpose:

- delete a user-managed profile.

Package, project read-only, or administrator-controlled profiles shall not be deleted through this method.

### 22.11 `reset_driver_configuration(path=None, scope="INSTANCE_OVERRIDE", apply=False, persist=False, confirm=False, alias=None)`
...
All standard configuration methods shall return only plain, JSON-serializable Python values:
...
`pathlib.Path`, dataclasses, enums, schema-validator objects, open files, exception objects, and other non-primitive Python models shall not be returned directly.
...
| Code | Meaning |
|---|---|
| `LPDS_CONFIG_SOURCE_NOT_FOUND` | Requested profile or file does not exist |
| `LPDS_CONFIG_ACCESS_DENIED` | Source or destination cannot be accessed |
| `LPDS_CONFIG_TOO_LARGE` | Input exceeds configured limit |
| `LPDS_CONFIG_ENCODING` | Input is not valid UTF-8 |
| `LPDS_CONFIG_JSON_SYNTAX` | Invalid JSON syntax |
| `LPDS_CONFIG_SCHEMA_ID` | Wrong schema or driver identity |
| `LPDS_CONFIG_SCHEMA_VERSION` | Unsupported schema version |
| `LPDS_CONFIG_SCHEMA_VALIDATION` | JSON Schema validation failed |
| `LPDS_CONFIG_UNKNOWN_KEY` | Unapproved key encountered |
| `LPDS_CONFIG_TYPE` | Wrong value type |
| `LPDS_CONFIG_RANGE` | Value outside allowed range |
| `LPDS_CONFIG_ENUM` | Unsupported enum value |
| `LPDS_CONFIG_SEMANTIC` | Cross-field rule failed |
| `LPDS_CONFIG_SECRET_UNRESOLVED` | Required secret reference cannot be resolved |
| `LPDS_CONFIG_SAFETY` | Safety rule rejected candidate |
| `LPDS_CONFIG_CONFIRMATION_REQUIRED` | Explicit authorization absent |
| `LPDS_CONFIG_DEVICE_IDENTITY` | Profile incompatible with connected device |
| `LPDS_CONFIG_MIGRATION_UNAVAILABLE` | No approved migration path |
| `LPDS_CONFIG_MIGRATION_FAILED` | Migration could not complete |
| `LPDS_CONFIG_LOCK_TIMEOUT` | Managed profile lock not acquired |
| `LPDS_CONFIG_CONFLICT` | Fingerprint or concurrent-write conflict |
| `LPDS_CONFIG_WRITE_FAILED` | Atomic persistence failed |
| `LPDS_CONFIG_INTEGRITY` | Fingerprint or schema-lock mismatch |
| `LPDS_CONFIG_APPLY_FAILED` | Runtime apply failed |
| `LPDS_CONFIG_ROLLBACK_FAILED` | Previous runtime state could not be restored |
| `LPDS_CONFIG_RECONNECT_REQUIRED` | Change cannot apply to active connection |
| `LPDS_CONFIG_RESTART_REQUIRED` | Driver restart required |
| `LPDS_CONFIG_DEVICE_WRITE_BLOCKED` | Device-persistent write not authorized |
| `LPDS_CONFIG_PROFILE_NAME` | Invalid managed profile identifier |
| `LPDS_CONFIG_PATH_POLICY` | Destination violates managed-path policy |

Configuration errors shall integrate with the LPDS unified exception and diagnostics standard (LPDS-007).

---

## 25. Capability Discovery Integration

LPDS-013 capability discovery shall identify configuration support.
...
## 26. LPDS-017 AI Driver Contract Integration

The LPDS-017 AI Driver Contract shall describe:

- the configuration persistence model;
- configuration-related public methods;
- risk levels;
...
## 27. LPDS-018 Test Bench Integration

The deployed LPDS-018 bench contract remains authoritative for:
...
Generic operator applications shall be able to construct configuration interfaces from `schema.json` and LPDS annotations.
...
## 29. LPDS-019 Conformance Integration

Configuration methods shall be included in the exported public API inventory.

For host-only configuration methods, LPDS-019 shall verify:
...
For configuration operations that apply device-facing settings, LPDS-019 shall additionally verify:
...
### 30.2 Python API tests

pytest-based tests against the driver's public API shall verify:

- all standard configuration methods are discoverable;
...
Where an adapter exists, a small adapter-native test suite shall additionally verify that its configuration-related bindings pass calls through to the corresponding public method unchanged.
...
- use LPDS-018 resource and safety data;
...
- operating system;
- Python version and, where applicable, the tested adapter's framework and version;
- tests executed;
- migrations tested;
...
At least one numbered plain-Python example shall demonstrate JSON round-trip configuration without real hardware.
...
- standard get and validate methods;
...
- complete unit, Python API, compatibility, security, and applicable HIL tests;
- configuration guide;
- GitHub Pages content;
- examples;
- evidence generation;
- LPDS-017 and LPDS-019 updates.
...
A driver passes LPDS-014 only when:
...
21. standard public configuration methods are discoverable and documented;
22. LPDS-013 capability discovery accurately reports configuration support;
23. LPDS-017 configuration semantics match the released API;
24. LPDS-019 verifies configuration method callability and applicable protocol behaviour;
...
- standard configuration methods return non-primitive, non-JSON-serializable objects;
...
Whenever a configuration field, default, schema constraint, annotation, environment variable, precedence rule, merge rule, persistence path, method, error, or migration changes, the same driver revision shall update:
...
- LPDS-013 capability data;
- LPDS-017 AI Driver Contract;
- LPDS-019 conformance vectors when device-facing behaviour changes;
...
24. Are standard public configuration methods exposed?
25. Does LPDS-013 accurately advertise configuration support?
26. Does LPDS-017 distinguish host persistence from device persistence?
27. Does LPDS-019 prove no unintended device operation occurs?
28. Are Python API, compatibility, security, and applicable HIL tests complete?
...
LPDS-014 implementation is complete for a driver when:
...
- standard public configuration methods are callable;
- capability discovery and AI contract are synchronized;
- applicable LPDS-019 vectors pass;
...
Provide one predictable configuration model across all LPDS drivers so that humans, plain Python callers, framework adapters, generic operator applications, and AI agents can validate, exchange, persist, migrate, inspect, and safely apply driver settings without relying on undocumented local behaviour.
...
{
  "lpds014_version": "1.0",
  "schema_id": "example_instrument.configuration",
  "schema_version": "1.0.0",
  "driver": {
  ...
  "settings": {
    ...
    "logging": {
      ...
      "output_directory": "${ENV:LPDS_RESULTS_DIR}"
    },
    "simulation": {
      "enabled": false,
      "profile": "default",
...
## Appendix B — Schema Fragment with LPDS Annotations

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "urn:lpds:example_instrument:configuration:1.0.0",
  "title": "LPDS Example Instrument Configuration",
  "type": "object",
  ...
  "properties": {
    ...
    "schema_id": {
      "const": "example_instrument.configuration"
    },
    "schema_version": {
      "const": "1.0.0"
    },
        ...
        "timeouts": {
          ...
          "properties": {
            "read_timeout_s": {
              ...
              "x-lpds-unit": "s",
              "x-lpds-apply-mode": "HOT",
              "x-lpds-risk-level": "NONE",
              "x-lpds-sensitive": false,
              "x-lpds-merge": "REPLACE"
            }
          }
        },
        "safety": {
          ...
          "properties": {
            "allow_device_persistent_changes": {
              "type": "boolean",
              "default": false,
              "x-lpds-apply-mode": "DEVICE_PERSISTENT",
              "x-lpds-risk-level": "HIGH",
              "x-lpds-requires-confirmation": true,
              "x-lpds-device-write": false
            }
          }
        }
      }
...
The following LPDS documents should be updated or interpreted consistently with LPDS-014:

1. **LPDS-001**
   Add the configuration manager and configuration resolution path to the platform architecture and traceability model.

2. **LPDS-002**
   Add the standard configuration methods and return schemas.

3. **LPDS-003**
   Assign shared configuration-manager responsibilities to `BaseInstrument`.

4. **LPDS-005**
   Canonical `default.json`, `example.json`, `schema.json`, and `schema.lock` are listed in LPDS-005 §6; `configuration.py`, profiles, and migration directories remain to be reflected there as the package layout evolves.

5. **LPDS-006**
   Add coding requirements for pure validation functions, type-safe models, redaction, atomic writes, and migration code.

6. **LPDS-007**
   Include the LPDS configuration error catalogue and rollback-state reporting.

7. **LPDS-008**
   Add configuration fingerprints, schema identity, redaction status, and migration evidence to the LPDS-008 diagnostic bundle and configuration evidence sections.
...
11. **LPDS-012**
    Require generic GUI generation from schema and LPDS annotations.
...
15. **LPDS-019**
    Include host-only configuration method callability, side-effect checks, and protocol proof for device-facing apply operations.
