...
**Version:** 1.0  
**Document ID:** LPDS-004  
**Status:** Draft project requirement  
**Applies to:** All LPDS Python instrument driver packages that communicate through VISA, serial, raw TCP/IP, or direct USB

---

## 1. Purpose

This specification defines the mandatory transport abstraction used by LPDS Python instrument drivers.
...
```text
Caller (plain Python, pytest, or a framework adapter)
        ↓
Driver public API
        ↓
Device protocol layer
        ↓
LPDS transport interface
        ↓
...
- asynchronous or synchronous serial-port communication exposed through a synchronous LPDS interface;
...
LPDS-004 does not define:

- the project-wide public driver API method names;
...
- **backend** — the implementation that adapts an external library or operating-system API to the LPDS transport interface;
...
Every LPDS transport implementation shall follow these principles.
...
All device communication shall pass through the LPDS transport interface or an approved capability extension.
...
Every backend shall support an observation mechanism sufficient to prove the actual outbound and inbound bytes or structured USB operation used by LPDS-019 conformance testing.

### 4.7 Backend substitution

A driver protocol implementation shall be testable with a simulator or spy transport without changing its public Python API.
...
A driver may combine files when the implementation is small, but the responsibilities shall remain separable and testable. Framework adapters are a separate concern from the driver and are addressed in §28; nothing in this document requires a driver to have any adapter at all.
...
Each LPDS driver that implements a transport shall provide equivalent files to:
...
A shared LPDS transport package may be used instead of duplicated driver-local code. When used, the driver shall pin or constrain a compatible transport-package version and record it in release evidence.
...
- session identifier generated by the LPDS transport;
- open timestamp;
...
Transport operations shall be implemented so that calling code — a plain Python caller, a pytest test, or a framework adapter — can regain control after the configured timeout.
...
An asynchronous backend may be used internally, but it shall expose the required synchronous behavior to the LPDS driver unless the project defines a separate asynchronous driver standard.
...
A backend may add more specific subclasses while preserving the stable LPDS category.

This `TransportError` hierarchy is internal to the transport boundary, not the public driver-facing hierarchy. At the protocol/driver boundary, transport failures shall be translated into the corresponding LPDS-007 `DriverError` categories (principally `DriverTransportError`, including `DriverTimeoutError` for the timeout subtree); LPDS-007 remains authoritative for how a translated failure is represented to callers, whether a plain Python caller or a framework adapter.
...
Protocol code and the driver's public API shall not need to catch backend-specific exception classes.
...
- which public driver method and protocol vector produced the exchange.
...
The LPDS VISA backend should use raw byte operations where available so that the canonical transport behavior remains byte exact.

When backend text operations are used, their encoding and termination behavior shall be proven equivalent to the LPDS codec configuration.
...
VISA status codes shall be translated to the LPDS error hierarchy while preserving the native code in machine-readable evidence.
...
Broken pipe, reset, aborted connection, and unreachable network errors shall map to stable LPDS categories.
...
Public driver API behavior shall not change silently merely because a different backend lacks an extension.
...
## 28. Framework Adapter Integration
...
When transport information is intentionally returned through a public diagnostic method on the driver's public API, values shall use plain Python scalar, list, dataclass, or dictionary types that any framework adapter can convert without special-casing.

Backend objects, exception instances, sockets, serial handles, VISA resources, USB device objects, locks, and byte iterators shall not be returned from the driver's public API, and therefore shall never reach a framework adapter.

### 28.3 Error reporting

Transport errors propagated out of the driver's public API shall preserve:
...
The driver's public API shall not expose an uncontrolled backend stack trace as its only diagnostic; no framework adapter should need to parse free-text messages to classify a failure.

---

## 29. Integration with LPDS-017 AI Driver Contract

Each LPDS-017 AI driver contract shall declare, as applicable:
...
- transport errors visible to the public API;
...
For each device-facing public method, the conformance environment shall be able to associate:
...
- method, vector, operation, and session correlation present.
...
| Error translation | Backend errors map to stable LPDS categories |
...
The contract suite should be reusable across driver packages or supplied by a shared LPDS transport package.
...
- **conformance trace** — full LPDS-019 evidence generation.
...
The transport setup guide shall include at least one plain-Python connection example for every supported transport type, and, where a framework adapter exists for the driver, a short labeled adapter usage example.
