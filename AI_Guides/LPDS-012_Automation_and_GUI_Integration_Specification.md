# LPDS-012 — Automation and GUI Integration Specification

**Document ID:** LPDS-012
**Version:** 1.0
**Status:** Draft Project Standard (Normative)
**Applies to:** Generic operator GUIs, automation and GUI integration adapters, bench-control applications, and LPDS driver packages claiming generic GUI or automation compatibility

---

## 1. Purpose

This specification defines the mandatory integration rules that allow a generic operator GUI, dashboard, or automation consumer to discover, display, configure, and safely operate LPDS Python instrument drivers without embedding vendor-specific protocol logic.

Any such consumer — a GUI, a web dashboard, a CLI operator console, or a test-automation framework — integrates with a driver either directly through its public Python API, or, when the consumer needs framework-specific bindings, through a thin adapter per LPDS-015. This specification uses "GUI" as its running example because it is the richest consumer, but every rule here applies equally to any generic automation consumer.

The GUI or consumer shall preserve the LPDS execution, metadata, safety, configuration, evidence, and conformance boundaries.
...
GUI controller and policy layer
      ↓
Approved public driver capability (public Python method, called directly or through an adapter)
      ↓
...
A GUI conforming to LPDS-012 shall be able to integrate a new LPDS driver primarily from machine-readable metadata and the stable public API, rather than from hard-coded knowledge of a specific manufacturer or model.
...
- loading and validating LPDS-017 driver contracts;
- loading LPDS-018 bench contracts when a deployed bench is controlled;
...
- automation suite and workflow execution, whether calling the driver's public API directly or through a framework adapter;
...
LPDS-012 does not define:

- vendor protocol syntax;
- transport implementation;
- the semantic behaviour of public driver methods;
- the complete LPDS-017 or LPDS-018 schema;
- physical bench wiring that is not declared in LPDS-018;
...
- replacement of any adapter's own native reporting;
...
LPDS-012 shall be applied together with the latest approved revisions of:
...
- **LPDS-013 — Capability Model**, when approved;
- **LPDS-014 — Driver Configuration Model Specification**, when approved;
- **LPDS-015 — Plugin and Adapter Architecture**, when approved;
- **LPDS-017 — AI Driver Contract Specification v3.0 or later**;
- **LPDS-018 — AI Test Bench Contract Specification**;
- **LPDS-019 — Driver Conformance Test Specification**;
- **LPDS-020 — Driver Implementation Lifecycle v1.1 or later**;
...
2. LPDS-018 deployed bench constraints and physical topology;
3. approved device-specific limitations;
4. LPDS-017 driver capability semantics;
5. the current public driver API and verified runtime state;
6. LPDS-012 GUI presentation rules;
...
- **GUI adapter** — the component that maps GUI requests to approved public driver calls and maps results or errors back to the GUI. This is distinct from an LPDS-015 framework adapter, though a GUI may itself be built on top of one.
- **Capability** — a declared public operation, normally corresponding to one public driver method.
...
- **Dangerous or destructive operation** — an operation whose LPDS-017 or bench classification requires special authorization, confirmation, fixture, backup, or recovery.
...
The GUI shall build its generic device model from LPDS contracts, runtime capability information, configuration schemas, and the public API. It shall not select behaviour by testing for a vendor, model name, Python class name, or private implementation symbol when a declared capability can be used.

### 5.2 Public API only

The GUI shall invoke only approved public driver capabilities, calling the driver's public Python API directly or through a declared adapter. It shall not:
...
LPDS-017 and the public driver API define operation semantics. LPDS-018 defines deployed bench topology and bench-wide constraints. GUI metadata may control presentation only.
...
│ Public method mapping, timeout, correlation, result mapping  │
└──────────────────────────────┬───────────────────────────────┘
                               │ public API only
┌──────────────────────────────▼───────────────────────────────┐
│ LPDS driver public Python API (directly, or via a framework  │
│ adapter when the consumer is itself framework-based)         │
└──────────────────────────────┬───────────────────────────────┘
                               │
┌──────────────────────────────▼───────────────────────────────┐
...
LPDS-017 + LPDS-018 + runtime capability manifest + config schema
+ LPDS-019 status/evidence + authorization policy
```
...
- exact mapping to the public capability (driver method, or the equivalent binding on a declared adapter);
...
|---|---|
| Public capability name and signature | Released public API and LPDS-017 lock-validated contract |
| Preconditions, postconditions, side effects, risk, timing, retry, errors | LPDS-017 |
| Physical topology, shared resources, signal routing, bench limits | LPDS-018 |
| Live connection, device identity, operation result, current state | Driver runtime result |
| Protocol-call qualification | LPDS-019 evidence |
...
- LPDS-017 generated-for version;
- public API manifest or lock;
- capability-manifest version;
- LPDS-018 referenced driver version.
...
- the GUI shall not attempt to repair the mismatch by guessing renamed arguments or methods.
...
- the package imports without hardware activity and with no automation framework installed;
- LPDS-017 loads and validates;
...
### 8.4 GUI-L3 — Automation workflow compatible

- automation suites and packaged examples can be selected and executed, either as plain Python/pytest calling the driver directly or, when one is declared, through a framework adapter;
- variables and profiles are validated before execution;
- the executing framework's standard reports are preserved;
- progress and stop behaviour are represented without false precision.

### 8.5 GUI-L4 — Bench-integrated compatible

- an LPDS-018 bench contract is loaded and validated;
...
- LPDS-019 evidence is current for enabled production capabilities;
...
- a valid LPDS-017 contract;
- a current contract lock or equivalent API-drift check;
- deterministic public method inventory;
- no hardware activity, and no automation-framework requirement, during import or metadata discovery;
...
- LPDS-019 status or an explicit unverified classification;
...
The GUI shall create one inventory entry for every public capability, including aliases and deprecated methods while they remain supported.
...
| Duration | Duration field accepting declared time units, normalized |
...
The test surface shall support approved test suites, examples, templates, variables, tags, profiles, output directory, execution status, and report links — whether the suite calls the driver's public API directly or runs through a framework adapter.

### 12.5 Results and evidence

The results surface shall provide run summaries, test reports (in whatever format the executing framework produces), measurements, attachments, exported configuration, version evidence, and cleanup status.
...
- driver import path and class name;
...
When LPDS-015 is available, the GUI shall use its approved plugin-discovery and loading mechanism (the `lpds.drivers` and, where relevant, `lpds.adapters` entry-point groups).
...
Importing a driver, reading its metadata, generating its documentation, or building a GUI manifest shall not:
...
- require any automation framework to be installed.
...
This is a GUI presentation-level projection, not an alternative state authority: it refines LPDS-003's canonical session-state enum (splitting `CONNECTED` into unverified/verified, and adding `UNKNOWN` for not-yet-queried GUI state). The GUI shall derive it deterministically from LPDS-003 states plus the GUI's own verification/query status, not define connection semantics independently.
...
The GUI shall represent resources declared by LPDS-017 and LPDS-018, including exclusive, shareable, read-only-shareable, multiplexed, externally locked, and unsafe-for-parallel resources.
...
Cross-driver concurrency shall be disabled when LPDS-018 does not prove that resources and signal paths are independent.
...
Hardware and automation-workflow execution shall not run on the presentation thread. The GUI shall remain responsive during connection, measurement, file transfer, calibration, reset, acquisition, and teardown operations.
...
- capability identifier and public method name;
...
- LPDS-017 preconditions;
- LPDS-018 topology and bench limits;
...
Repeated, ramped, scanned, or multi-channel actions shall use an approved driver capability or workflow. The GUI shall not synthesize an unbounded loop over a single active method without explicit rate, limit, stop, and cleanup controls.
...
## 19. Automation Workflow and Test Execution

### 19.1 Canonical execution

An automation suite calling the driver's public API directly shall be executed through a standard Python/pytest entry point. When a framework adapter is used instead, the suite shall be executed through that framework's own supported entry point, and the GUI shall preserve that framework's exit status and standard artifacts unchanged.
...
- required adapter and its version, when the suite depends on one;
- LPDS-018 bench contract when applicable;
...
The GUI shall show determinate progress only when a reliable total and completed count are available. Otherwise it shall show an indeterminate running state plus current suite, test, method, phase, or operation when available.

### 19.4 Failure handling

The GUI shall preserve the original test failure and separately report teardown, cleanup, report-generation, or export failures.

### 19.5 Result authority

The executing framework's own PASS/FAIL and declared verification oracles shall remain authoritative. The GUI shall not reinterpret a failed test as passed or invent a pass threshold not present in the approved suite or contract.
...
Safety rules shall be taken from LPDS-017, LPDS-018, approved device-specific requirements, and deployed configuration. Presentation hints shall never weaken them.
...
When LPDS-018 defines an emergency workflow, the GUI shall provide a continuously accessible emergency control while the affected bench is active.
...
- automation run start, completion, and artifact locations.

### 25.2 Correlation

Every operation, error, result, and protocol or automation-run evidence reference shall carry a correlation identifier sufficient to reconstruct the sequence.

### 25.3 Version evidence

Each GUI session or automation run shall record, as applicable:

- GUI version;
- driver package version;
- adapter name and version, when a framework adapter was used;
- LPDS-017 contract version and lock/hash;
- LPDS-018 bench contract version/hash;
- Python version and, where applicable, the executing framework's version;
- operating system;
- selected profile;
...
LPDS-008 §11 is the sole normative source for the canonical result-directory layout and artifact roles. The GUI-specific layout above is an illustrative mapping onto that canonical layout (for example, `gui_session.json` corresponds to LPDS-008's `run_summary.json`, and `operation_events.jsonl` corresponds to LPDS-008's `events/operations.jsonl`); a GUI implementation may instead write directly into the LPDS-008 canonical paths, and shall do so when both are produced for the same run to avoid duplicate, divergent evidence roots.
...
The presentation event loop shall not perform blocking hardware, file, network, automation-run, or report-generation work.
...
Only approved and identifiable driver and adapter plugins shall be loaded. Plugin installation or enablement shall require appropriate authorization and shall be recorded.
...
│       └── test_gui_integration.py   (plus an adapter-native suite, if an adapter is used)
├── docs/
│   └── gui_integration.md
└── review/
...
Equivalent locations are permitted until LPDS-005 is revised, provided they are documented and included in release validation.
...
The build shall fail when the generated GUI manifest does not match the current public API and LPDS-017 contract.
...
- connection profiles;
- manual and automation-workflow execution behaviour;
...
- LPDS-017 and LPDS-018 validation;
...
### 32.7 Automation workflow execution tests
...
- the executing framework's standard artifacts;
- live status mapping;
...
GUI-L4 tests shall use an LPDS-018 test bench or approved deterministic bench simulator to verify topology, operator actions, resource conflicts, sequencing, stabilization, emergency workflow, and safe shutdown.
...
| Field | Requirement |
|---|---|
| Capability | Public method/capability identifier |
| Canonical method | Canonical public operation |
...
| LPDS-019 status | Qualified, partial, excluded, unverified |
...
A GUI integration passes LPDS-012 only when all applicable criteria pass.
...
3. The installed public API, LPDS-017 contract, and generated GUI manifest are version-consistent.
...
9. Blocking driver and automation-run operations do not block the presentation thread.
...
13. Multi-driver operation uses the deployed LPDS-018 contract.
...
18. The executing framework's exit status and standard reports are preserved.
19. Every operator action and operation is traceable through correlation identifiers and version evidence.
20. LPDS-019 qualification status is displayed without overstating simulator or HIL evidence.
...
LPDS-012 shall fail when any of the following applies:
...
- LPDS-018 topology or operator action is assumed but not declared;
...
- an automation run's failure is hidden or reclassified as success;
...
LPDS-012 shall be implemented through the LPDS phase-and-gate lifecycle (LPDS-020).
...
- LPDS-018 bench integration where in scope.
...
- full static, simulator, GUI, automation-workflow, negative, recovery, accessibility, and security tests;
...
- automation-workflow execution review;
...
- LPDS-017 contract and lock;
- LPDS-018 bench references;
- public API manifest;
...
- examples and automation workflows;
...
5. Does metadata discovery avoid hardware access and any automation-framework requirement?
...
19. Is LPDS-018 loaded for multi-driver hardware operation?
...
30. Are the executing framework's exit status and reports preserved?
...
LPDS-012 is complete for a driver/GUI integration when:

- the target GUI compatibility level is declared;
- the public capability inventory is complete;
- LPDS-017 and, where applicable, LPDS-018 load and validate;
...
- manual and automation-workflow execution paths preserve safety and evidence;
...
Provide a stable, metadata-driven, safe, testable, and evidence-producing operator GUI (or other generic automation consumer) integration model in which any conforming LPDS driver can be loaded and operated through its declared public capabilities — directly or through a declared adapter — without embedding device-specific protocol knowledge in the generic consumer.

---

## Appendix A — Minimal Generated GUI Manifest Example

The exact schema may be refined by LPDS-013, LPDS-014, and LPDS-015. The following example defines the required intent and is not an independent semantic authority.

```json
{
  "schema": "lpds-012-gui-manifest-1.0",
  "generated": {
    "timestamp": "2026-07-26T12:00:00Z",
    "tool_version": "1.0.0",
    "sources": {
      "driver_package": "example_v26.01",
      "driver_version": "26.1",
      "lpds017_version": "3.0",
      "lpds017_hash": "<sha256>",
  ...
  "capabilities": [
    {
      ...
      "risk_reference": "LPDS017:CAP-SET-VOLTAGE",
      "resources": ["psu_session", "channel:<channel>"],
      "timeout_s": 10,
      "cleanup_reference": "LPDS017:CAP-DISABLE-OUTPUT",
      "qualification": {
        "lpds019": "QUALIFIED",
        "hil": "VERIFIED"
      }
...
```json
{
  ...
  "evidence": {
    "automation_run": null,
    "protocol_trace": "lpds019://CAP-SET-VOLTAGE/vector-01",
